...
let appData = {
	...
	detectDayBudget: function() {                                            
		...
		alert ("Ежедневный бюджет:  " + appData.moneyPerDay + "денег.");
	},
	detectLevel: function() {
		if(appData.moneyPerDay < 100){
			...
			console.log("Высокий уровень достка");
		} else {
	...
	chooseOptExpenses: function() {
		for (let i = 1; i <= 3; i++) {
			let questionOptExpenses = prompt("Статья необезательних расходов");
	...
	chooseIncome: function() {
		...
		for( let o = 0; o < 1; o++) {
			if( (typeof(items)) != null && (typeof(items)) === "string" && items != "") {
				...
				appData.income.push(prompt("Может штото еще?", ""));
				appData.income.sort();
			} else {
